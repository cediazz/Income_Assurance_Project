export default async function getCommercialOperationsIncomes(date1,date2){
    ...
    try {
      let config = {
        ...
        let res = await fetch(`http://localhost:8000/reportes/CommercialOperations/get_incomes/?date1=${date1}&date2=${date2}`, config)
        data = await res.json()
        console.log(data)
        if(res.status == 401){
          console.log("401")
          ...
          let res2 = await fetch(`http://localhost:8000/reportes/CommercialOperations/get_incomes/?date1=${date1}&date2=${date2}`, config)

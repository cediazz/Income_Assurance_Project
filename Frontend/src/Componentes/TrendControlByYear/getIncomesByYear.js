export default async function getIncomesByYear(year1,year2){
    ...
    try {
      let config = {
        ...
        let res = await fetch(`http://localhost:8000/reportes/CommercialOperations/get_incomes_yearly/?year1=${year1}&year2=${year2}`, config)
        data = await res.json()
        console.log(data)
        if(res.status == 401){
          console.log("401")
          ...
          let res2 = await fetch(`http://localhost:8000/reportes/CommercialOperations/get_incomes_yearly/?year1=${year1}&year2=${year2}`, config)
